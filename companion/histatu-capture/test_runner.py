...
# ---------- detection-issue report capture ----------
# capture is a no-op when the session is expired or full (never touches PIL then)
def _boom_strip(*a):
    raise AssertionError("_strip must not be called when the session is expired/full")
_dbgself = types.SimpleNamespace(hud=types.SimpleNamespace(_strip=_boom_strip),
                                 cfg={"window_title": "Hytale"})
_dbgself._debug_log = lambda d, line: hr.App._debug_log(_dbgself, d, line)
_dbgself._debug = {"until": time.time() - 1, "frames": [], "bytes": 0, "res": "", "log": ""}  # expired
hr.App._debug_capture(_dbgself, None, {})  # must return early, no exception
t("debug capture: expired session captures nothing", _dbgself._debug["frames"] == [])
_dbgself._debug = {"until": time.time() + 60, "frames": [1] * hr.DEBUG_MAX_FRAMES, "bytes": 0, "res": "", "log": ""}
hr.App._debug_capture(_dbgself, None, {})  # full -> early return
t("debug capture: full session captures nothing more", len(_dbgself._debug["frames"]) == hr.DEBUG_MAX_FRAMES)
_dbgself._debug = {"until": time.time() + 60, "frames": [], "bytes": hr.DEBUG_MAX_BYTES, "res": "", "log": ""}
hr.App._debug_capture(_dbgself, None, {})  # byte-capped -> early return
t("debug capture: byte-capped session captures nothing more", _dbgself._debug["frames"] == [])

# game window not locatable -> grab_game() fell back to the DESKTOP; pixels must NOT ship
_dbgself._debug = {"until": time.time() + 60, "frames": [], "bytes": 0, "res": "", "log": ""}
_orig_fgw = hr.find_game_window
hr.find_game_window = lambda title: None
try:
    _desktop = hr.Image.new("RGB", (1920, 1080))
    hr.App._debug_capture(_dbgself, _desktop, {"position": None, "target": None})
finally:
    hr.find_game_window = _orig_fgw
t("debug capture: window not found -> no pixels, only a log note",
  _dbgself._debug["frames"] == [] and "not located" in _dbgself._debug["log"]
  and "pixels NOT captured" in _dbgself._debug["log"])

# window found: strip JPEG ships + the session log carries ONLY panel-strip OCR text
_dbgself.hud = types.SimpleNamespace(_strip=lambda f: f)
_dbgself._debug = {"until": time.time() + 60, "frames": [], "bytes": 0, "res": "", "log": ""}
hr.find_game_window = lambda title: (0, 0, 100, 80)
try:
    hr.App._debug_capture(_dbgself, hr.Image.new("RGB", (100, 80)),
                          {"position": (1.0, 2.0, 3.0), "target": None,
                           "strip_texts": ["Position 1.00 2.00 3.00"],
                           "texts": ["Position 1.00 2.00 3.00", "FULLFRAME chat leak"]})
finally:
    hr.find_game_window = _orig_fgw
t("debug capture: window found -> frame recorded", len(_dbgself._debug["frames"]) == 1
  and _dbgself._debug["frames"][0]["jpg"].startswith("data:image/jpeg;base64,"))
t("debug capture: session log has strip OCR only — never full-frame rescan text",
  "Position 1.00" in _dbgself._debug["log"] and "FULLFRAME" not in _dbgself._debug["log"])

# finish with nothing recorded just resets and reports; never spawns an upload
_fin = types.SimpleNamespace(_debug={"until": 0, "frames": [], "bytes": 0, "res": "", "log": ""},
                             statuses=[])
_fin.set_status = lambda m, c=None: _fin.statuses.append(m)
hr.App._finish_debug(_fin)
t("debug finish: no frames -> session cleared, no upload", _fin._debug is None
  and any("no chest presses" in s for s in _fin.statuses))

# a stale 180s timer (its session already finished manually) must not end a NEWER session
_fin2 = types.SimpleNamespace(statuses=[], set_status=lambda m, c=None: None)
_old = {"until": 0, "frames": [], "bytes": 0, "res": "", "log": ""}
_new = {"until": time.time() + 120, "frames": [], "bytes": 0, "res": "", "log": ""}
_fin2._debug = _new
hr.App._finish_debug(_fin2, expected=_old)
t("debug finish: stale timer leaves the newer session running", _fin2._debug is _new)

# the uploaded bundle carries the log + frames but NEVER the editor write key
_SECRET = "SUPER-SECRET-EDITOR-KEY-123"
_up = types.SimpleNamespace(cfg={**hr.DEFAULT_CONFIG, "ign": "Rev", "write_key": _SECRET,
                                 "hotkey_chest": "F", "ocr_poll_sec": 1.8}, mode="log", statuses=[])
_up.set_status = lambda m, c=None: _up.statuses.append(m)
_posted = {}
class _FakeResp:
    def __enter__(self): return self
    def __exit__(self, *a): return False
    def read(self): return b'{"id":"dbg-test"}'
_orig_urlopen = hr.urllib.request.urlopen
hr.urllib.request.urlopen = lambda req, timeout=0: (_posted.update(data=req.data), _FakeResp())[1]
_orig_open = builtins.open
def _no_disk_open(path, *a, **k):
    if "read_debug" in str(path):
        raise AssertionError("upload must NOT read read_debug.log — it can hold full-frame text")
    return _orig_open(path, *a, **k)
builtins.open = _no_disk_open
try:
    hr.App._upload_debug(_up, [{"note": "pos=None", "jpg": "data:image/jpeg;base64,AAAA"}],
                         "2560x1369", "[12:00:00] pos=None target=None\n    ocr: Position 1.00")
finally:
    hr.urllib.request.urlopen = _orig_urlopen
    builtins.open = _orig_open
_body = _posted["data"].decode()
t("debug upload: bundle sent with the frame", '"frames"' in _body and "AAAA" in _body)
t("debug upload: editor key NEVER in the bundle", _SECRET not in _body)
t("debug upload: session log (strip-only) included", "ocr: Position 1.00" in _body)
t("debug upload: useful metadata included", '"resolution": "2560x1369"' in _body and "IGN=Rev" in _body)
t("debug upload: success status shown", any("report sent" in s.lower() for s in _up.statuses))

# ---------- editions: lite / full ----------
# lite build: the detection reporter never runs — clicking it offers the Full upgrade instead
_lite = types.SimpleNamespace(_debug="SHOULD-NOT-TOUCH", offered=[])
_lite._offer_full_upgrade = lambda: _lite.offered.append(True)
_lite._finish_debug = lambda *a, **k: (_ for _ in ()).throw(AssertionError("lite must not capture"))
_orig_edition = hr.IS_LITE
hr.IS_LITE = True
try:
    hr.App._start_debug(_lite)
finally:
    hr.IS_LITE = _orig_edition
t("lite: report button offers Full upgrade, captures nothing", _lite.offered == [True])
